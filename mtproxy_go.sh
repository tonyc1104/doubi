...
check_new_ver(){
	new_ver=$(wget -qO- https://api.github.com/repos/9seconds/mtg.git| grep "tag_name"| head -n 1| awk -F ":" '{print $2}'| sed 's/\"//g;s/,//g;s/ //g')
...
Download(){
	...
	wget --no-check-certificate -N "https://github.com/9seconds/mtg.git ${new_ver}/mtg-linux-${bit}"
